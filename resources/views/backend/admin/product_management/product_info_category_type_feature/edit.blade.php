    ...
    <div class="row">
        <div class="{{ $document ? 'col-md-8' : 'col-md-12' }}">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('pm.pro-info-cat-tf.update', encrypt($feature->id)) }}" method="POST"
                        ...
                        <div class="form-group">
                            ...
                            <select name="product_info_cat_id" class="form-control" id="product_info_cat_id">
                                ...
                                @foreach ($pro_info_cats as $cat)
                                    <option value="{{ $cat->id }}"
                                        {{ $feature->pro_info_cat_id == $cat->id ? 'selected' : '' }}>
                                        {{ $cat->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" value="{{ old('name', $feature->name) }}" id="title" name="name"
                                class="form-control" placeholder="Enter name">
                            <x-feed-back-alert :datas="['errors' => $errors, 'field' => 'name']" />
                        </div>
                        <div class="form-group">
                            <label>{{ __('Slug') }}<span class="text-danger">*</span></label>
                            <input type="text" value="{{ old('slug', $feature->slug) }}" id="slug" name="slug"
...
@push('js')
    <script>
        $(document).ready(function() {
            ...
            getInfoCatTypes(`{{ $feature->product_info_cat_id }}`, route,
                `{{ $feature->product_info_cat_type_id }}`);
        });
    </script>
    {{-- FilePond  --}}
@endpush
