...
@section('content')
    ...
    <form action="{{ route('frontend.checkout-order.submit', $order->order_number) }}" method="POST">
        @csrf
        <div class="container">
            <div class="flex flex-col lg:flex-row gap-8 my-10">
                ...
                <div class="lg:w-2/3 bg-bg-white p-6 shadow-card rounded-lg dark:bg-bg-dark-tertiary">
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
                                <div>
                                    <input type="text" name="first_name" placeholder="First Name"
                                        value="{{ old('first_name') }}" class="input h-10">
                                    <x-frontend.input-error :datas="['errors' => $errors, 'field' => 'first_name']" />
                                </div>
                                <div>
                                    <input type="text" name="last_name" placeholder="Last Name"
                                        value="{{ old('last_name') }}" class="input h-10">
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
                                <div>
                                    <div class="flex flex-wrap lg:flex-row gap-2">
                                        @foreach (App\Models\PersonalInformation::getLanguages() as $key => $lang)
                                            <div class="flex items-center">
                                                <input type="radio" name="language" id="language_{{ $key }}"
                                                    value="{{ $key }}"
                                                    {{ old('language') == $key ? 'checked' : '' }}>
                                                <label for="language_{{ $key }}"
                                                    class="ml-2 text-text-gray">{{ $lang }}</label>
                                            </div>
                                        @endforeach
                                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
                        ...
                        <div class="">
                            <select name="country_id" id="country" class="input h-10">
                    ...
                    <div class="grid grid-cols-1 xl:grid-cols-3 gap-4 mb-6">
                        <div>
                            <select name="state_id" id="state" disabled class="input h-10">
                                <option value="" selected hidden>{{ __('Select State') }}</option>
                            </select>
                            <x-frontend.input-error :datas="['errors' => $errors, 'field' => 'state_id']" />
                        </div>
                        <div>
                            <select name="city_id" id="city" disabled class="input h-10">
                        ...
                        <div>
                            <input type="text" name="postal_code" value="{{ old('postal_code') }}"
                                placeholder="Zipcode/Postal" class="input h-10">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
                        ...
                        <div>
                            {{-- Detination --}}
                            <select name="destination_port" id="destination" class="input">
                                <option value="" selected disabled>{{ __('Select Destination Port') }}</option>
                                @foreach ($shipping_locations as $destination_port)
                                    <option value="{{ $destination_port->id }}"
                                        {{ old('destination_port') == $destination_port->id ? 'selected' : '' }}>
                                        {{ $destination_port->name }}
                                    </option>
                                @endforeach
                            </select>
                            <x-frontend.input-error :datas="['errors' => $errors, 'field' => 'destination_port']" />
                        </div>
                    </div>
                    <div class="mb-4">
                        <textarea name="address_line_1" class="textarea no-ckeditor5" id="address" placeholder="Address">{{ old('address_line_1') }}</textarea>
                    ...
                    <div class="space-y-4 mb-8">
                        @foreach (App\Models\Order::getContainerTypeLabels() as $key => $value)
                            <div
                                ...
                                <input type="radio" id="shipping_method_{{ $key }}" name="container_type"
                                    class="w-4" value="{{ $key }}"
                                    {{ old('container_type') == $key ? 'checked' : '' }}>
                                <label for="shipping_method_{{ $key }}" class="w-full">
                                    <div class="flex justify-between">
                                        <span>{{ $value }}</span>
                                    </div>
                                </label>
                                <x-frontend.input-error :datas="['errors' => $errors, 'field' => 'container_type']" />
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="lg:w-1/3">
                    <div class="sticky top-20 bg-bg-white p-6 shadow-card dark:bg-bg-dark-tertiary rounded-lg">
                        ...
                        <x-frontend.primary-button :href="false" bg="false" button="true" type="submit"
...
@push('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            // Event listeners
            $('input[name="first_name"], input[name="last_name"]').on('input', updateName);
            $('input[name="email"]').on('input', updateEmail);
            $('input[name="whatsapp"]').on('input', updatePhone);
