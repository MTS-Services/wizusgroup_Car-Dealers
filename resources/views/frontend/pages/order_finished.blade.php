                ...
                <div class="flex items-center">
                    ...
                    <div>
                        @if (isset($container) && $container)
                            <h3 class="font-semibold text-lg">{{ __('🚢 Container Join Confirmation!') }}</h3>
                            <p class="text-sm">
                                {{ __('You have successfully joined the container. We will contact you soon with further details.') }}
                            </p>
                        @else
                            <h3 class="font-semibold text-lg">{{ __('📦 Container Request Submitted!') }}</h3>
                            <p class="text-sm">
                                {{ __('Your container request has been received. We will arrange a suitable container and contact you within 24-48 hours.') }}
                            </p>
                        @endif
                    </div>
                </div>
            ...
            <div id="voucher" class="bg-white border-2 border-gray-300 rounded-lg shadow-xl p-8 mb-8">
                ...
                <div class="text-center border-b-2 border-gray-200 pb-6 mb-6">
                    <h1 class="text-3xl font-bold text-gray-800 mb-2">
                        @if (isset($container) && $container)
                            {{ __('CONTAINER JOIN VOUCHER') }}
                        @else
                            {{ __('CONTAINER REQUEST VOUCHER') }}
                        @endif
                    </h1>
                    <div class="text-lg text-gray-600">
                        ...
                        <p>{{ __('Date') }}: <span
                                class="font-semibold">{{ $order->created_at->format('F d, Y') }}</span></p>
                        <p>{{ __('Status') }}: <span
                ...
                @if (isset($container) && $container)
                    <div class="bg-green-50 p-4 rounded-lg mb-6 border-l-4 border-green-500">
                        <h3 class="font-semibold text-lg mb-3 text-gray-800">{{ __('🚢 Container Information') }}</h3>
                        <div class="grid md:grid-cols-2 gap-4 text-sm">
                            <div>
                                <p><span class="font-medium">{{ __('Container') }}:</span>
                                    {{ $container->title ?? 'N/A' }}</p>
                                <p><span class="font-medium">{{ __('Type') }}:</span>
                                    {{ $order->container_type_label ?? 'N/A' }}</p>
                                <p><span class="font-medium">{{ __('Dimensions') }}:</span>
                                    {{ $container->length_m ?? 0 }}m × {{ $container->width_m ?? 0 }}m ×
                                    {{ $container->height_m ?? 0 }}m</p>
                            </div>
                            <div>
                                <p><span class="font-medium">{{ __('Max Weight') }}:</span>
                                    {{ number_format($container->max_weight_kg ?? 0) }} kg</p>
                                <p><span class="font-medium">{{ __('Base Cost') }}:</span>
                                    ${{ number_format($container->base_cost ?? 0, 2) }}</p>
                                <p><span class="font-medium">{{ __('Per CBM Cost') }}:</span>
                                    ${{ number_format($container->per_cbm_cost ?? 0, 2) }}</p>
                            </div>
                        </div>
                    </div>
                @else
                ...
                @php
                    ...
                    $reservePrice = 0;
                    if (isset($container) && $container) {
                        $containerPrice = $container->per_cbm_cost * $totalCBM + $container->base_cost;
                ...
                @if (isset($container) && $container && $containerPrice > 0)
                    <div class="bg-blue-50 p-4 rounded-lg mb-6 border-l-4 border-blue-500">
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span>${{ number_format($container->base_cost, 2) }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span>{{ __('CBM Cost') }} (${{ number_format($container->per_cbm_cost, 2) }} ×
                                    {{ number_format($totalCBM, 2) }} m³):</span>
                                <span>${{ number_format($container->per_cbm_cost * $totalCBM, 2) }}</span>
                            </div>
                            <hr class="my-2">
                            <div class="flex justify-between font-semibold text-lg">
                                <span>{{ __('Total Container Price') }}:</span>
                                <span class="text-red-600">${{ number_format($containerPrice, 2) }}</span>
                            </div>
                    ...
                    <div class="bg-blue-100 p-6 rounded-lg border-2 border-blue-300">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex justify-between text-lg">
                                ...
                                <span class="font-bold">${{ number_format($containerPrice, 2) }}</span>
                            </div>
                            <hr class="border-blue-300">
                            <div class="flex justify-between text-2xl font-bold text-blue-800">
                                <span>{{ __('Grand Total') }}:</span>
                                <span>${{ number_format($order->total + $containerPrice, 2) }}</span>
                            </div>
                        </div>
                    </div>
                @endif
                ...
                <div class="bg-yellow-50 p-4 rounded-lg mt-6 border-l-4 border-yellow-500">
                    ...
                    <div class="text-sm space-y-1">
                        <p>• {{ __('Email') }}: support@{{ config('app.name', 'yourcompany') }}.com</p>
                        <p>• {{ __('Phone') }}: +1 (555) 123-4567</p>
                        <p>• {{ __('WhatsApp') }}: +1 (555) 123-4567</p>
                        <p>• {{ __('Business Hours') }}: {{ __('Monday - Friday, 9:00 AM - 6:00 PM') }}</p>
                    </div>
                </div>
