<section>
    ...
    <header
        ...
        <div class="max-w-7xl mx-auto flex flex-col md:flex-row flex-wrap items-center justify-between gap-3 sm:gap-4">
            ...
            <div class="flex flex-col lg:flex-row items-start md:items-center gap-3 md:gap-4 w-full lg:w-auto">
                ...
                <div class="w-full lg:w-auto flex justify-between items-center">
                    <a href="{{ url('/') }}" class="flex-shrink-0">
                        <img src="{{ settings('site_logo') ? storage_url(settings('site_logo')) : asset('frontend/images/logo.png') }}"
                            alt="Logo" class="w-24 sm:w-28 h-auto">
                    </a>
                    <!-- Toggle Buttons -->
                    <div class="flex items-center gap-3 lg:hidden">
                        <div class="tablet:hidden w-[50%] flex justify-end">
                            <a href="{{ route('login') }}"
                                class="bg-bg-black text-center w-full xs:w-40 text-white px-4 sm:px-6 py-2 text-xs sm:text-sm hover:bg-bg-dark-tertiary/90 transition-colors rounded">
                                Login
                            </a>
                        </div>
                        <button id="mobile-search-toggle"
                            class="text-gray-700 dark:text-white focus:outline-none text-xl">
                            <i class="fas fa-search"></i>
                        </button>
                        <button id="mobile-menu-toggle"
                            class="text-gray-700 dark:text-white focus:outline-none text-2xl">
                            ☰
                        </button>
                    </div>
                </div>
                ...
                <!-- Mobile Search Bar -->
                <div id="mobile-search-bar" class="hidden md:hidden w-full mt-3">
                    <div class="flex w-full">
                        ...
                        <button
                            class="bg-bg-primary text-white px-3 py-2 rounded-r hover:bg-bg-primary/80 transition-colors">
                            <i class="fas fa-search"></i>
                        </button>
                    </div>
                </div>

                <!-- Info: Japan Time + Desktop Search (hidden on mobile) -->
                <div class="hidden lg:flex flex-col gap-2 w-full lg:w-auto">
            ...
            <div class="hidden lg:flex flex-col xs:flex-row items-center gap-2 w-full xs:w-auto justify-end">
                <div class="flex flex-row gap-2 w-full xs:w-auto">
                    <a href="{{ route('register') }}"
                        class="bg-bg-wiz_orange w-full xs:w-44 text-center text-white py-2 text-xs sm:text-sm font-medium hover:bg-bg-wiz_orange/80 transition-colors rounded">
                        Member Registration
                    </a>
                    <a href="{{ route('login') }}"
                        class="bg-bg-black text-center w-full xs:w-40 text-white px-4 sm:px-6 py-2 text-xs sm:text-sm hover:bg-bg-dark-tertiary/90 transition-colors rounded">
            ...
            <!-- Mobile Login Button (below search on mobile) -->
    ...
    <nav class="hidden lg:block bg-bg-primary dark:bg-bg-dark-tertiary text-white shadow-md">
        <div class="max-w-7xl mx-auto px-4">
            <div class="flex tablet:justify-between items-center">
                <div id="menu-items"
                    class="hidden xs:flex flex-wrap items-center w-full xs:w-auto space-x-0 sm:space-x-1">
                    <a href="{{ url('/') }}"
                        class="py-4 px-3 sm:px-4 lg:pl-1 tablet:px-6 xl:px-10 hover:bg-blue-800 transition-colors font-medium text-sm tablet:text-base">
                        <span>Home ▼</span>
                    </a>
                    ...
                    <a href="{{ route('frontend.contact') }}"
                        class="py-4 px-3 sm:px-4 lg:pl-1 tablet:pl-6 xl:px-10 hover:bg-blue-800 transition-colors font-medium text-sm tablet:text-base">
                        <span>Contact ▼</span>
                    </a>
                </div>
            </div>
    ...
    <div id="mobile-menu" class="hidden lg:hidden bg-bg-primary dark:bg-bg-dark-tertiary text-white px-4">
        ...
        <a href="{{ route('frontend.group_shipping') }}" class="block py-3 border-b border-white/10 text-sm">Group
            Shipping ▼</a>
        <a href="{{ route('frontend.products') }}" class="block py-3 border-b border-white/10 text-sm">Products ▼</a>
        <a href="{{ route('frontend.parts-accessories') }}" class="block py-3 border-b border-white/10 text-sm">Parts &
            Accessories ▼</a>
